...
parser.add_argument('-c', '--config', type=str, default='./train_config.yaml',
...
model = UNet3D(in_channels=opt_model['in_chans'], out_channels=opt_model['out_chans'], final_sigmoid=False, layer_order='cr')

model.to(device)
optimizer = Adam(model.parameters(), lr=opt_train['optimizer_lr'], betas=(opt_train['optimizer_beta1'], opt_train['optimizer_beta2']))
